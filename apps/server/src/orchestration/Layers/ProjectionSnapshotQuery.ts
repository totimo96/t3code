...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
    ...
    execute: () =>
      sql`
        SELECT
          ...
          COALESCE(workspace_kind, 'coding') AS "workspaceKind",
          branch,
  ...
  const listDesignBriefRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionDesignBriefDbRowSchema,
    execute: () =>
      sql`
        SELECT
          thread_id AS "threadId",
  ...
  const getDesignBriefRowByThread = SqlSchema.findOneOption({
  ...
  const listDesignArtifactRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionDesignArtifactDbRowSchema,
    execute: () =>
      sql`
        SELECT
          thread_id AS "threadId",
  ...
  const getDesignArtifactRowByThread = SqlSchema.findOneOption({
    Request: ThreadIdLookupInput,
    Result: ProjectionDesignArtifactDbRowSchema,
    execute: ({ threadId }) =>
      sql`
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listDesignBriefRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listDesignBriefs:query",
                "ProjectionSnapshotQuery.getSnapshot:listDesignBriefs:decodeRows",
              ),
            ),
          ),
          listDesignArtifactRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listDesignArtifacts:query",
                "ProjectionSnapshotQuery.getSnapshot:listDesignArtifacts:decodeRows",
              ),
            ),
          ),
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              for (const row of designBriefRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
                designBriefByThread.set(row.threadId, {
              ...
              for (const row of designArtifactRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
                designArtifactByThread.set(row.threadId, {
                  html: row.html,
                  version: row.version,
                  updatedAt: row.updatedAt,
                });
      ...
      .withTransaction(
        Effect.all([
          ...
          listDesignBriefRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listDesignBriefs:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listDesignBriefs:decodeRows",
              ),
            ),
          ),
          listDesignArtifactRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listDesignArtifacts:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listDesignArtifacts:decodeRows",
              ),
            ),
          ),
          listDesignAssetRows(undefined).pipe(
        ...
        Effect.flatMap(
          ([
            ...
            Effect.sync(() => {
              ...
              for (let index = 0; index < designBriefRows.length; index += 1) {
                const row = designBriefRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.updatedAt);
              ...
              for (let index = 0; index < designArtifactRows.length; index += 1) {
                const row = designArtifactRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.updatedAt);
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    Effect.gen(function* () {
      const [
        ...
        getDesignBriefRowByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:getDesignBrief:query",
              "ProjectionSnapshotQuery.getThreadDetailById:getDesignBrief:decodeRow",
            ),
          ),
        ),
        getDesignArtifactRowByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:getDesignArtifact:query",
              "ProjectionSnapshotQuery.getThreadDetailById:getDesignArtifact:decodeRow",
            ),
          ),
        ),
      ...
      const thread = {
        ...
        designBrief: Option.isSome(designBriefRow)
          ? {
              markdown: designBriefRow.value.markdown,
              version: designBriefRow.value.version,
              updatedAt: designBriefRow.value.updatedAt,
            }
          : null,
        designArtifact: Option.isSome(designArtifactRow)
          ? {
              html: designArtifactRow.value.html,
              version: designArtifactRow.value.version,
